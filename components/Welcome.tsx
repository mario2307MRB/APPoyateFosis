...
const ProjectsCarousel: React.FC = () => {
    ...
    return (
        <div className="w-full max-w-3xl mx-auto mt-10">
            <h2 className="text-2xl font-bold text-gray-700 mb-4 text-center">Proyectos Recientes</h2>
            <div className="relative bg-slate-50 rounded-lg shadow-lg overflow-hidden">
                <div className="relative h-56 flex items-center justify-center transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
                    {recentProjects.map((project) => {
                        ...
                        <div key={project.id} className="w-full flex-shrink-0 p-6 md:p-8 text-left box-border">
                            ...
                            <p className="mt-2 text-gray-600"><strong>Monto:</strong> {formatCurrency(project.amount)}</p>
                            ...
                            <div className="mt-4">
                              ...
                              <div className="w-full bg-gray-200 rounded-full h-2.5">
                                <div className="bg-fosis-green h-2.5 rounded-full" style={{ width: `${percentageRendered > 100 ? 100 : percentageRendered}%` }}></div>
                              </div>
                            </div>
                        </div>
                    )})}
                ...
                {recentProjects.length > 1 && (
                    <>
                        <button onClick={prevProject} className="absolute top-1/2 left-2 -translate-y-1/2 bg-white/60 hover:bg-white rounded-full p-2 transition shadow-md">
                            <ChevronLeftIcon className="w-6 h-6 text-fosis-blue" />
                        </button>
                        <button onClick={nextProject} className="absolute top-1/2 right-2 -translate-y-1/2 bg-white/60 hover:bg-white rounded-full p-2 transition shadow-md">
                            <ChevronRightIcon className="w-6 h-6 text-fosis-blue" />
                        </button>

                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2">
                            {recentProjects.map((_, index) => (
                                <button key={index} aria-label={`Ir al proyecto ${index + 1}`} onClick={() => setCurrentIndex(index)} className={`w-3 h-3 rounded-full transition ${currentIndex === index ? 'bg-fosis-blue' : 'bg-gray-300 hover:bg-gray-400'}`}></button>
                            ))}
                        </div>
                    </>
                )}
...
const Welcome: React.FC<WelcomeProps> = ({ user }) => {
    return (
        <div className="w-full flex flex-col items-center p-4 sm:p-6 lg:p-8">
            <div className="bg-white p-6 sm:p-10 rounded-2xl shadow-2xl w-full max-w-4xl mx-auto text-center">
