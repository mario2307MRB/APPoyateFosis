...
const TaskManagerView: React.FC = () => {
    ...
    const handleSaveGame = () => {
        // This button provides explicit user feedback. The useLocalStorage hook saves automatically.
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800">Planificador de Tareas Interactivo</h1>
                    <p className="text-gray-600 mt-1">Asigna tareas y acumula puntos según la prioridad y duración.</p>
                </div>
                <div className="flex items-center gap-4">
                     <div className="text-right p-2 rounded-lg bg-slate-200">
                        <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Puntaje</span>
                        <p className="text-3xl font-bold text-fosis-green-dark">{score}</p>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-2">
                        <button
                            onClick={handleSaveGame}
                            className="flex items-center justify-center gap-2 px-4 py-2 bg-fosis-green text-white rounded-lg shadow-md hover:bg-fosis-green-dark transition-colors"
                        >
                        ...
                        <button
                            ...
                            className="flex items-center justify-center gap-2 px-4 py-2 bg-fosis-blue text-white rounded-lg shadow-md hover:bg-fosis-blue-dark transition-colors"
                        >
                            <PlusCircleIcon className="w-5 h-5"/>
                            <span>Generar Nuevas Tareas</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="mb-8">
                <h2 className="text-2xl font-bold text-gray-700 mb-4">Banco de Tareas Pendientes</h2>
                <div className="bg-white p-4 rounded-xl shadow-lg max-h-72 overflow-y-auto">
                    {pendingTasks.length > 0 ? (
                        ...
                        <p className="text-center text-gray-500 py-8">¡Excelente! No hay tareas pendientes.</p>
                    )}
                </div>
            </div>
                ...
                <div key={week.weekNumber}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                        {week.days.map((day, dayIndex) => (
                            <div key={dayIndex} className="bg-white p-4 rounded-xl shadow-lg flex flex-col min-h-[300px]">
                                ...
                                <div className="my-2">
                                    ...
                                    <div className="w-full bg-gray-200 rounded-full h-2.5"><div className="bg-fosis-green h-2.5 rounded-full" style={{ width: `${(day.usedHours / day.capacity) * 100}%` }}></div></div>
                                </div>
                                <div className="space-y-2 mt-2 pt-2 border-t flex-grow overflow-y-auto min-h-[150px]">
                                    {day.tasks.map(task => (
                                        <div key={task.id} className={`p-2 rounded-md text-sm border ${getPriorityClasses(task.priority)} relative group`}>
                                            ...
                                            <button onClick={() => handleRemoveTask(task, weekIndex, dayIndex)} className="absolute top-1 right-1 text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><TrashIcon className="w-4 h-4"/></button>
                                        </div>
                                    ))}
                                </div>
                                <button onClick={() => handleOpenModal(weekIndex, dayIndex)} className="mt-2 w-full flex items-center justify-center gap-2 text-sm p-2 bg-slate-100 hover:bg-slate-200 rounded-md text-fosis-blue-dark font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed" disabled={day.usedHours >= day.capacity}>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col">
                         <div className="p-4 border-b flex justify-between items-center">
                            ...
                            <button onClick={handleCloseModal} className="text-gray-400 hover:text-gray-600"><XCircleIcon className="w-7 h-7"/></button>
                        </div>
                        <div className="p-4 overflow-y-auto">
                            {availableTasksForModal.length > 0 ? (
                                <div className="space-y-3">
                                {availableTasksForModal.map(task => (
                                    <button key={task.id} onClick={() => handleAssignTask(task)} className={`w-full text-left p-3 border rounded-lg transition-transform hover:scale-105 hover:shadow-md ${getPriorityClasses(task.priority)}`}>
                                        <p className="font-semibold">{task.title}</p>
                                        <div className="flex justify-between items-center mt-2 text-sm">
                                            <span className={`px-2 py-0.5 rounded-full font-medium text-xs ${getPriorityClasses(task.priority)} border`}>Prioridad {task.priority}</span>
                                            <span className="font-bold">{task.durationHours}h</span>
                                ...
                                <p className="text-center text-gray-500 py-10">No hay tareas disponibles que se ajusten al tiempo restante para este día.</p>
                            )}
                        </div>
                    </div>
                </div>
            ...
            {showSaveMessage && (
                <div className="fixed bottom-8 right-8 bg-gray-800 text-white py-3 px-5 rounded-lg shadow-xl z-50">
