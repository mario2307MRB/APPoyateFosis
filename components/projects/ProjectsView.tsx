    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <h1 className="text-3xl font-bold text-fosis-blue-900">Mis Proyectos</h1>
                <button
            ...
            <div className="mb-6 bg-white p-4 rounded-xl shadow-soft-lg border border-slate-200">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 items-end gap-4">
                    <div className="sm:col-span-2">
                        <label htmlFor="search" className="block text-sm font-medium text-slate-700 mb-1">Buscar por Código o Ejecutor</label>
                        <input
                            type="text"
                            id="search"
                            ...
                            className="block w-full border border-slate-300 rounded-md shadow-soft-sm p-2 focus:ring-fosis-blue-700 focus:border-fosis-blue-700"
                            aria-label="Buscar proyectos"
                        />
                    </div>
                    <div>
                        <label htmlFor="startDate" className="block text-sm font-medium text-slate-700 mb-1">Inicio Desde</label>
                        <input
                            type="date"
                            id="startDate"
                    ...
                    <div>
                        <label htmlFor="endDate" className="block text-sm font-medium text-slate-700 mb-1">Término Hasta</label>
                        <input
                            type="date"
                            id="endDate"
            ...
            {projects.length > 0 ? (
                filteredProjects.length > 0 ? (
                    ...
                    <div className="text-center py-16 bg-white rounded-2xl shadow-soft-md border">
                        <h2 className="text-xl font-semibold text-slate-700">No se encontraron proyectos</h2>
                        <p className="text-slate-500 mt-2">Intenta ajustar tus filtros o términos de búsqueda.</p>
                    </div>
                )
            ) : (
                <div className="text-center py-16 bg-white rounded-2xl shadow-soft-md border">
                    <h2 className="text-xl font-semibold text-slate-700">Aún no hay proyectos ingresados</h2>
                    <p className="text-slate-500 mt-2">¡Comienza agregando tu primer proyecto para verlo aquí!</p>
                </div>
            )}
